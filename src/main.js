...
const DEFAULT_TODOS = [
  {
    ...
    description:
      "Notice the smooth transitions and card layouts designed for clarity.",
    completed: true,
    priority: "high",
    category: "work",
  ...
  },
  {
    ...
    description:
      "Click and hold any task to drag it into your preferred order.",
    completed: false,
    priority: "high",
    category: "work",
    dueDate: new Date().toISOString().split("T")[0],
  ...
  },
  {
    ...
    description:
      "Get bell peppers, chicken breasts, fresh basil, and some olive oil.",
    completed: false,
    priority: "medium",
    category: "shopping",
  ...
  },
  {
    ...
    description:
      "Draft initial wireframes for a dashboard layout focusing on minimalist grids and generous spaces.",
    completed: false,
    priority: "low",
    category: "ideas",
...
function renderTodoList() {
  ...
  filteredTodos.forEach((todo) => {
    ...
    // Drag Start: Store task ID and configure drag styles
    ...
    // Drag End: Reset local styles and clear all hover markers
    ...
    // Drag Over: Necessary to permit dropped actions and visual target indicator
    ...
    // Drag Enter: Prevent standard defaults
    card.addEventListener("dragenter", (e) => {
      e.preventDefault();
    });

    // Drag Leave: Erase visual drag-over indicator
    card.addEventListener("dragleave", () => {
      card.classList.remove("drag-over");
    });

    // Drop Event: Reorder underlying tasks index state & commit update
...
function bootstrapApp() {
  ...
  if (editTaskForm) {
    editTaskForm.addEventListener("submit", (e) => {
      ...
      if (index !== -1 && editTitle) {
        ...
        todos[index].dueDate =
          editDue.value || new Date().toISOString().split("T")[0];
  ...
  // Undo Button Trigger inside Notification Toast
  ...
  // Backup & Recovery drag & drop and manual file selection listeners
        ...
        if (Array.isArray(importedData)) {
          const validatedTasks = importedData.map((task) => {
            return {
              ...
              title:
                typeof task.title === "string" ? task.title : "Unnamed task",
              description:
              ...
              priority: ["low", "medium", "high"].includes(task.priority)
                ...
                : "medium",
              category:
                typeof task.category === "string" ? task.category : "personal",
              dueDate:
